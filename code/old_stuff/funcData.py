...
def _descartar(feat,i,n):
    ...
    if i < n:
        ...
        feat['goals'] = _np.NaN
        feat['shots'] = _np.NaN
        feat['on_target'] = _np.NaN
        feat['corners'] = _np.NaN
        feat['fouls'] = _np.NaN    
...
def crear_dif_tend(data):
    ...
    return _pd.DataFrame(new_data)

def mergeDB(rawDB,fdataDB,data_cuotas=data_cuotas):

    with open('equipos._json', 'r') as eq:
        eqs = _json.load(eq)
    ...
    for dc in data_cuotas:
        cuotasFeat[dc] = _np.empty(shape=(len(rawDB)),dtype=object)
