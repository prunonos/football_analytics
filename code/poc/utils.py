...
from pagerank import create_graph
random.seed(0)
...
def read_data(path,date=["Date"],date_format="%d/%m/%Y"):
    return pd.read_csv(path,sep=';',decimal=',',parse_dates=date,date_format=date_format,low_memory=False)

def save_dataframe(df,path,name_of_file,**atr):
    df.to_csv(f"{path}{name_of_file}.csv",decimal=',',sep=';',**atr)
...
def compute_lag(df:pd.DataFrame,lag,min_samples,col_window,cols_group,aggregations,cols_agg=False,closed='left',keep=False):
    if cols_agg==False: cols_agg = [ k+"_"+str(lag) for k in aggregations.keys() ]
    if keep: df_keep = df.loc[:,keep+cols_group+[col_window]].set_index(cols_group+[col_window])
    df_agg = (df.groupby(cols_group)
        .rolling(window=lag,min_periods=min_samples,on=col_window,closed=closed)
        .agg(aggregations)
    )
