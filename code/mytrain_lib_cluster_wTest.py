...
def train_model(model, criterion, optimizer, dataloader_train,
    ...
    for ep in range(epochs):
        ...
        if display and ((ep+1)%(epochs/10)==0 or ep==0):
            ...
            if cv>-1: print(f'\rProgress:{ (cv*100/5)+ (ep*20/epochs) }%.')
        ...
        if ep==epochs-1: accuracy_test.append(acc_test)  
...
def train_wCrossValidation(config,train_data,test_data,kfold,epochs=5,display=True,bat_size=32,
    ...
    for fold,(train_idx,test_idx) in enumerate(kfold.split(train_data.data)):
        ...
        error_fold,acc_train_fold,acc_test_fold = (
    ...
    return np.array(error), np.array(accuracy_train), np.array(accuracy_val), np.array(accuracy_test), np.array(confusion_matrix)
        

##################################

def save_score(error,accuracy_train,accuracy_val,accuracy_test,confusion_matrix,hyperparams,temp,root='',title=''):
    if root=='': 
    ...
    np.save(root+'accval'+title,accuracy_val)
...
def Grid_Search_SGD(train_data,test_data,scalers,criterion,learning_rate,momentum,
                model,kfold,nesterov=False,
                batch_size=32, epochs=100,root='', save_outputs=True, display=False):

    error, accuracy_train, accuracy_val, accuracy_test, confusion_matrix = [],[],[],[],[]
    ...
    for c,hyper in enumerate(hyperparams):
        ...
        # test dataloader
        testloader = testloader = DataLoader(test_data,batch_size=hyper[5],shuffle=True)

        # training model
        er, ac_tr, ac_val, ac_te, cm = train_wCrossValidation(config, train_data, testloader, kfold, epochs, display=False,
                                    confnum=c,logs=True,save_outputs=save_outputs,path=root+'//')

        error.append(er), accuracy_train.append(ac_tr), accuracy_val.append(ac_val)
        confusion_matrix.append(cm), accuracy_test(ac_te) 
        
        if display:
            print('\rConfig: {}/{}: loss train: {:.2f}, accuracy train: {:.2f}, accuracy val.: {:.2f}, accuracy test: {:.2f}'.
                format(c+1, len(hyperparams), np.mean(er), np.mean(ac_tr),
                                                            np.mean(ac_val), np.mean(ac_te)), end='')
        else: print(f'\nConfig:{c} de {len(hyperparams)}.\n')

        if save_outputs: save_logging(temp,title=str(c),root=path_outputs+root+'//')
        
    save_score(error,accuracy_train,accuracy_val,accuracy_test,confusion_matrix,
                    hyperparams,temp=temp,root=path_scores+root+'//',title='')

    return error,accuracy_train,accuracy_val,accuracy_test,confusion_matrix

######## ADAM #########

def Grid_Search_Adam(train_data,test_data,scalers,criterion,learning_rate,b1,b2,
    ...
    for c,hyper in enumerate(hyperparams):
        ...
        # test dataloader
        testloader = DataLoader(test_data,batch_size=hyper[5],shuffle=True)
