...
class WyscoutDataset(Dataset):
    def __init__(self,file):
        ...
        # if file=='train': df, lab_df = self._add_test(df,lab_df)
    ...
    def _add_test(df_train,labels,p=0.1):
        df_test = pd.read_csv(path_wyscout+'X_'+'test'+'.csv',sep=';')#.sample(frac=p,random_state=1)
        lab_test = pd.read_csv(path_wyscout+'y_'+'test'+'.csv',sep=';')#.merge(test['wyId'], on='wyId', how='inner')
        print(type(df_test), type(df_train))
        df_train = pd.concat([df_train,df_test],axis=1)
        labels = pd.concat([labels,lab_test],axis=1)
        assert df_train.shape[0] == labels.shape[0]
...
class FootballMatchesDataset(Dataset):
    def __init__(self,file,dataset,drop=[],factor=-1,regression=False):
        ...
        if regression:
            ...
            dif_result      = (np.abs(df['FTHG'].astype(float)-df['FTAG'].astype(float))+1).to_numpy()
...
def test_model(model,dataloader_test,isCE,return_preds=False):
    model.eval()
    with torch.no_grad():
        acc_run = 0
        total_len   = 0
    ...
    acc_test = acc_run / total_len
    if return_preds: return acc_test, confusion_matrix, preds_array
