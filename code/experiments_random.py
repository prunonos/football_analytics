...
group_dimred.add_argument("-feat", action="store_true")         # iterar
...
parser.add_argument('-wd','--weight_decay',type=float,nargs='*',default=None)
...
print(f'\nID={os.getpid()} ---- {datetime.now().strftime("%m.%d %H:%M:%S")} ---- INFO: Start of Execution.')
print(f'ID={os.getpid()} ---- {datetime.now().strftime("%m.%d %H:%M:%S")} ---- INFO: Using {device} device.')
...
if args.dataset=='wyscout':
    ...
    train_data  = ml.FootballMatchesDataset('train',args.dataset,drop=args.drop,factor=args.factor,regression=args.regression)
    test_data   = ml.FootballMatchesDataset('test',args.dataset,drop=args.drop,factor=args.factor,regression=args.regression)
    drop_string = 'drop_'
...
for a in args.activation:
    if 'relu'==a: activations.append((torch.nn.functional.relu,0))
...
def generate_lossweights(lw):
    if lw is None: return np.round(np.random.random((20,3))*4,2).tolist()
    else: return lw

def generate_model_scenarios(neuralnets,units):
    print(f'ID={os.getpid()} ---- {datetime.now().strftime("%m.%d %H:%M:%S")} ---- INFO: Generating model scenarios.')

    scenarios = []
    output_classes = 2 if args.regression else 3

    units = generate_units(units)
    lossweights = generate_lossweights(args.lossweights)
    for w in lossweights:
        for act in activations:
            for hn in units:
                for net in neuralnets:
                    if 'dumb'==net: 
                        scenarios.append({'model':gn.dumbmodelclass, 
                                    'params':{'hidden_neurons':hn.tolist(), 'output_classes':output_classes,'activation':act[0], 
                                    'activ_opt':{'negative_slope':act[1]}, 'loss_weights': torch.tensor(w), 'initialization':args.init},
                        ...
                        for p in np.random.uniform(low=.0, high=.2, size=4):
                            scenarios.append({
                                'model':gn.NeuralNetworkDOClass, 
                                'params':{'hidden_neurons':hn.tolist(), 'p': p, 'output_classes':output_classes,'activation':act[0], 
                                'activ_opt':{'negative_slope':act[1]}, 'loss_weights': torch.tensor(w), 'initialization':args.init}
                                })
    return scenarios
...
def generate_optimizer_scenarios():
    print(f'ID={os.getpid()} ---- {datetime.now().strftime("%m.%d %H:%M:%S")} ---- INFO: Generating optimizer scenarios.')

    scenarios = []
    if args.weight_decay==None: 
        weight_decay = [0,0,0,0,0,*np.random.uniform(low=.0, high=1, size=5)]
    else: 
        weight_decay = args.weight_decay
    
    if args.sgd or not args.adam:
        momentum = np.random.uniform(low=.0, high=1.0, size=5)
        nesterov = [0]
        dampening = [0,*np.random.uniform(low=.0, high=.5, size=4)]
    ...
    if args.adam or not args.sgd:
        b1, b2 =  [.001,.9,*np.random.uniform(low=.0, high=1.0, size=3)], [.01,.99,*np.random.uniform(low=.0, high=1.0, size=3)]
        hyper = (np.array(np.meshgrid(weight_decay,b1,b2,args.lr)).T.reshape((-1,4)))
...
def run_HypeTune(title,train_data,test_data):
    scenarios = {'train_data':train_data, 'test_data':test_data, 'root':title, 'save_outputs':args.outputs, 'maxruns':args.maxruns,
                    'batch_size':args.batch,'epochs':args.epochs,'folds':args.folds, 'scalers':scalers, 'display':False , 
                    'criterion':criterions, 'factor':args.factor}

    scenarios = generate_scenarios(scenarios)
    print(f"ID={os.getpid()} ---- {datetime.now().strftime('%m.%d %H:%M:%S')} ---- INFO: Running experiment.")
    ml.Tuning(**scenarios)
...
if type(args.anova)==list:
    ...
    for n_feat in args.anova:
        ...
        train_data.features = old_feat[mask_new_feat]
        
        if old_train.shape[1]>n_feat: run_HypeTune(title,train_data,test_data)
        else: print(f'ID={os.getpid()} ---- {datetime.now().strftime("%m.%d %H:%M:%S")} ---- {title} ---- {n_feat} es mayor que {len(old_train)}.\n')

elif type(args.pca)==list:
    # procesamiento de pca

    for nDim in args.pca:
        title = f'{args.t}_pca_{nDim}'
        ...
        else: print(f'ID={os.getpid()} ---- {datetime.now().strftime("%m.%d %H:%M:%S")} ---- {title} ---- {nDim} es mayor que {len(old_train)}.\n')

elif args.feat:
    title = f'{args.t}_featsel'
    ...
    for thres in args.varthr:
        title = f'{args.t}_varthr_{thres}'
        filter          = feature_selection.VarianceThreshold(thres).fit(X_norm)
        mask_new_feat   = filter.get_support()
        train_data.data = old_train[:,mask_new_feat]
        test_data.data  = old_test[:,mask_new_feat]
        train_data.features = old_feat.columns[mask_new_feat]
        if len(train_data.features)>0: run_HypeTune(title,train_data,test_data)
        else: print(f'ID={os.getpid()} ---- {datetime.now().strftime("%m.%d %H:%M:%S")} ---- {title} ---- No hay variable seleccionadas.\n')
...
print(f'ID={os.getpid()} ---- {datetime.now().strftime("%m.%d %H:%M:%S")} ---- INFO: End of Execution.\n')