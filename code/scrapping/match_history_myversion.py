...
def parse_dates(date_series, time_series):
    datetime_str = date_series + ' ' + time_series
    
    # Try the primary format first
    try:
        return pd.to_datetime(datetime_str, format="%d/%m/%Y %H:%M", errors='raise')
    except ValueError:
        pass
    ...
    # As a fallback, try to infer the format
    ...
    def read_games(self) -> pd.DataFrame:
        ...
        for lkey, skey in itertools.product(self._selected_leagues.values(), self.seasons):
            ...
            try:
                ...
                print(f"Error processing {lkey}, {skey}: {e}")
        ...
        df = (
            ...
            .assign(
                date=lambda x: parse_dates(x["date"], x["time"])
            )
            .drop("time", axis=1)
